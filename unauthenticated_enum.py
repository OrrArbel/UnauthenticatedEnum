...
WINRMS_PORT = 5986
OS_DICT = {
    10: {0: "Windows 10/Windows Server versions 1903/1909/2004"},
    6: {
    ...
    5: {
        0: "Windows 2000",
        2: "Windows Server 2003/R2 /Windows XP Professional x64 Edition",
        1: "Windows XP",
    },
}
...
def parse_ntlm_challenge(challenge, translateOS=False):
    ...
    Parameters:
       translateOS: Print OS name in friendly format
    ...
    if ntlmChallenge["TargetInfoFields_len"] > 0:
        ...
        if av_pairs[ntlm.NTLMSSP_AV_DOMAINNAME] is not None:
            try:
                ...
                print(
                    "NetBIOS domain name (Computer name if not domain joined): %s"
                    % server_domain
                )
            except UnicodeDecodeError:
                # For some reason, we couldn't decode Unicode here.. silently discard the operation
        ...
        if av_pairs[ntlm.NTLMSSP_AV_DNS_DOMAINNAME] is not None:
            try:
                ...
                print(
                    "Domain FQDN (Computer name if not domain joined): %s"
                    % server_dns_domain_name
                )
            except UnicodeDecodeError:
                # For some reason, we couldn't decode Unicode here.. silently discard the operation
                pass
...
def winrms_collector(target):
    ...
    prepped.headers["Authorization"] = (
        "Negotiate TlRMTVNTUAABAAAAMZCI4gAAAAAoAAAAAAAAACgAAAAGAbEdAAAADw=="
    )
    ...
    try:
        ...
        )  # verify=False is for debugging with self-signed certificate
    except Exception as e:
        print(f"WinRM ould not get response from target - {e}")
        return False

    # Extract the NTLM challenge by removing the "Negotiate" prefix and decoding from Base64
    if "WWW-Authenticate" in response.headers:
        challenge = b64decode(
            response.headers["WWW-Authenticate"].split("Negotiate ")[1]
        )
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Enunmerate hosts without authenticating."
    )
    parser.add_argument(
        "ip",
        type=str,
        help="Address or address range to scan, netmask required for address range (examples: single ip - 10.1.1.12 , class c semgment - 10.1.1.0/24)",
    )
